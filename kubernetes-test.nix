let
  ...
  corednsPod = pkgs.dockerTools.pullImage {
  ...
  pausePod = pkgs.dockerTools.pullImage {
  ...
  etcdPod = pkgs.dockerTools.pullImage {
  ...
  kubeapiPod = pkgs.dockerTools.pullImage {
  ...
  kubeschedulerPod = pkgs.dockerTools.pullImage {
  ...
  kubecontrollerPod = pkgs.dockerTools.pullImage {
  ...
  ciliumPod = pkgs.dockerTools.pullImage {
...
pkgs.testers.nixosTest {
  ...
  nodes.machine =
    ...
    {
      imports = [
        (import ./helsinki-kubernetes.nix)
      ];
  ...
  testScript = ''
    ...
    machine.succeed("${pkgs.containerd}/bin/ctr -n k8s.io image import ${pausePod}")
    machine.succeed("${pkgs.containerd}/bin/ctr -n k8s.io image import ${corednsPod}")
    machine.succeed("${pkgs.containerd}/bin/ctr -n k8s.io image import ${etcdPod}")
    machine.succeed("${pkgs.containerd}/bin/ctr -n k8s.io image import ${kubeapiPod}")
    machine.succeed("${pkgs.containerd}/bin/ctr -n k8s.io image import ${kubecontrollerPod}")
    machine.succeed("${pkgs.containerd}/bin/ctr -n k8s.io image import ${kubeschedulerPod}")
    machine.succeed("${pkgs.containerd}/bin/ctr -n quay.io image import ${ciliumPod}")
